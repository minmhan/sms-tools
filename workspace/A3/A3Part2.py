...
def optimalZeropad(x, fs, f):
    ...
    """
    ## Your code here
    length = len(x) # length of samples
    periods = fs/f # number of samples in one period
    M = length
    if length % periods != 0:
	M = length + (periods - (length % periods)) # minimum zero padding


    fftbuffer = np.zeros(M)
    fftbuffer[:length] = x[:length]
    X = fft(fftbuffer)
